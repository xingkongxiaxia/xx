...
class SHAN(SequentialRecommender):
    ...
    def __init__(self,config,dataset):
        ...
        self.long_b=nn.Parameter(uniform_(tensor=torch.zeros(self.embedding_size),a=-np.sqrt(3 / self.embedding_size), b=np.sqrt(3 / self.embedding_size)),requires_grad=True).to(self.device)
        self.long_short_w=nn.Linear(self.embedding_size,self.embedding_size)
        self.long_short_b=nn.Parameter(uniform_(tensor=torch.zeros(self.embedding_size),a=-np.sqrt(3 / self.embedding_size), b=np.sqrt(3 / self.embedding_size)),requires_grad=True).to(self.device)
    ...
    def forward(self,seq_item,user,seq_item_len):
        ...
        long_term_attention_based_pooling_layer=self.long_term_attention_based_pooling_layer(seq_item_embedding,user_embedding,mask)
        ...
        long_short_item_embedding=self.long_and_short_term_attention_based_pooling_layer(long_short_item_embedding,user_embedding,mask_long_short)
    ...
    def long_and_short_term_attention_based_pooling_layer(self,long_short_item_embedding,user_embedding,mask=None):
        ...
        :param long_short_item_embedding: batch_size * 1_plus_short_item_length * embedding_size
        :param user_embedding: batch_size * embedding_size
        :param mask: batch_size * 1_plus_short_item_length
        :return:
        """
        ...
        long_short_item_embedding=torch.mul(long_short_item_embedding_value,long_short_item_embedding.unsqueeze(2)).sum(dim=1)
    ...
    def long_term_attention_based_pooling_layer(self,seq_item_embedding,user_embedding,mask=None):
        ...
        == long_w1 ==>> batch_size * seq_len * embedding_size
        :param user_embedding: batch_size * embedding_size
        ...
        user_item_embedding=torch.mul(seq_item_embedding_value,user_item_embedding.unsqueeze(2)).sum(dim=1,keepdim=True)
        # batch_size * 1 * embedding_size
